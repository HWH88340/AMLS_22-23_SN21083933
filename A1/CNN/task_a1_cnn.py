...
def read_picture(train_flag):
    # filequeue
    if train_flag == 1:
        file_name_list = glob.glob("./Datasets/celeba/img/*.jpg")
    else:
        file_name_list = glob.glob("./Datasets/celeba_test/img/*.jpg")
...
def task_a1_cnn(train_flag):
    ...
    # prepare data
    x = tf.placeholder(tf.float32, shape=[None, 218, 178, 3])
    ...
    with tf.Session() as sess:
        ...
        if train_flag == 1:
            ...
            for i in range(2000):
                ...
                if i % 30 == 0:
                    saver.save(sess, "./model_A1/Task_A1_model")
                if error < 0.01:
                    break
            ...
            # load the model
            if os.path.exists("./model_A1/checkpoint"):
                saver.restore(sess, "./model_A1/Task_A1_model")
            # start the coord
            coord = tf.train.Coordinator()
            threads = tf.train.start_queue_runners(sess=sess, coord=coord)
