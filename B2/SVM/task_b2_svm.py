...
def task_b2_svm(train_flag):
    if train_flag:
        ...
        for inputfile in glob.glob("./Datasets/cartoon_set_test/img/*.png"):
            outputdir = "./b2_svm_processed_eyes_test"
            mkdir(outputdir)
            img_convert(inputfile, outputdir)
            j = j + 1
            print(j, "times")

        path = "./b2_svm_processed_eyes_test"
    ...
    if train_flag == 1:
        ...
        with open('b2_svm.pickle', 'wb') as w:
            pickle.dump(model, w)
        ...
        with open('b2_svm.pickle', 'rb') as r:
