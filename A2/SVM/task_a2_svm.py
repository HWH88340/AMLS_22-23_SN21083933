...
def task_a2_svm(train_flag):
    if train_flag == 1:
        ...
        for inputfile in glob.glob("./Datasets/celeba_test/img/*.jpg"):
            outputdir = "./a2_svm_processed_img_test"
            mkdir(outputdir)
            img_convert(inputfile, outputdir)
            j = j + 1
            print(j, "times")

        path = "./a2_svm_processed_img_test"
    ...
    if train_flag == 1:
        ...
        with open('a2_svm.pickle', 'wb') as w:
            pickle.dump(model, w)
        ...
        with open('a2_svm.pickle', 'rb') as r:
